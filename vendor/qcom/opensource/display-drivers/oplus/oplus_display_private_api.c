...
int oplus_request_power_status = OFP_REQUEST_NO;
...
int oplus_ofp_on_vblank = -1;
int oplus_ofp_off_vblank = -1;
		...
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d %d %d %d\n", oplus_dimlayer_bl_on_vblank,
			oplus_dimlayer_bl_off_vblank, oplus_ofp_on_vblank, oplus_ofp_off_vblank,
		...
		const char *buf, size_t count)
{
	sscanf(buf, "%d %d %d %d", &oplus_dimlayer_bl_on_vblank,
			...
			&oplus_ofp_on_vblank, &oplus_ofp_off_vblank, &oplus_datadimming_v3_debug_value,
...
static ssize_t oplus_display_get_dc_bl(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	unsigned int dc_bl = 0;
	struct dsi_display *display = get_main_display();
	...
	if (!strcmp(display->panel->oplus_priv.vendor_name, "BF092_AB241")) {
                if (dc_apollo_enable) {
                        if (display->panel->bl_config.bl_level > JENNIE_DC_THRESHOLD)
                                dc_bl = display->panel->bl_config.bl_level;
                        else
                                dc_bl = display->panel->bl_config.bl_dc_real;
                } else
                        dc_bl = display->panel->bl_config.bl_level;
        }
	return sprintf(buf, "%d\n", dc_bl);
...
static OPLUS_ATTR(dc_bl, S_IRUGO|S_IWUSR, oplus_display_get_dc_bl, NULL);
