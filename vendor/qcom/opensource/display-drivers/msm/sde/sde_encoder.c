...
#if defined(CONFIG_PXLW_IRIS) || defined(CONFIG_PXLW_SOFT_IRIS)
#define DELAY_LOW_FPS_IMEOUT_US 10000
#define DELAY_MEDIUM_FPS_IMEOUT_US 5500
#define DELAY_HIGHT_FPS_IMEOUT_US 1000
			...
			struct msm_display_info *disp_info)
{
	...
	if (!sde_enc->cur_master) {
		SDE_ERROR("invalid param sde_enc->cur_master:%d", sde_enc->cur_master != NULL);
		return;
	}

	if (!sde_enc->cur_master->connector) {
		SDE_ERROR("invalid param sde_enc->cur_master->connector:%d", sde_enc->cur_master->connector != NULL);
		return;
	}
...
static void sde_encoder_off_work(struct kthread_work *work)
{
	...
	if (oplus_adfr_is_support() && oplus_adfr_idle_mode_is_enable()) {
		SDE_DEBUG("kVRR enter idle mode\n");
...
/* Add for backlight smooths */
	...
	if (oplus_oha_is_support()) {
		SDE_DEBUG("kOFP, oplus_ofp_update_oha\n");
		oplus_ofp_update_oha(sde_enc->cur_master->connector);
	}
#endif
...
void sde_encoder_wait_vblack(struct drm_connector *connector, struct drm_encoder *drm_enc, int wait_num)
{
	...
	if (wait_vsync_flag > 0) {
		if (panel_bl_dirty) {
			SDE_ATRACE_BEGIN("wait_actual_vblack");
			/* wait wait_num times TE to make sure set backlight in frame start */
			while (wait_num) {
				sde_encoder_wait_for_event(drm_enc, MSM_ENC_VBLANK);
				wait_num--;
			}
			SDE_ATRACE_END("wait_actual_vblack");
		}

		fps = sde_encoder_get_fps(drm_enc);
		if (fps < 75) {
			usleep_range(DELAY_LOW_FPS_IMEOUT_US, DELAY_LOW_FPS_IMEOUT_US + 10);
		} else if (fps > 75 && fps < 100) {
			usleep_range(DELAY_MEDIUM_FPS_IMEOUT_US, DELAY_MEDIUM_FPS_IMEOUT_US + 10);
		} else {
			usleep_range(DELAY_HIGHT_FPS_IMEOUT_US, DELAY_HIGHT_FPS_IMEOUT_US + 10);
		}
	}
}
