...
static struct wireless_chg_debug_info g_chg_debug_info = {
	.tx_version = 0,
	.rx_version = 0,
	.boot_version = 0,
	.dock_version = 0,
...
static void oplus_wpc_update_chg_debug_info(struct oplus_chg_wls *wls_dev, bool clear)
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	struct wireless_chg_debug_info *chg_debug_info = &g_chg_debug_info;
	int batt_temp = 250;
	u32 rx_version = 0, trx_version = 0;
	int cep_curr_ma = 0;

	if (clear) {
		memset(chg_debug_info, 0x0, sizeof(struct wireless_chg_debug_info));
		return;
	}

	oplus_chg_wls_get_batt_temp(wls_dev, &batt_temp);
	chg_debug_info->highest_temp = max(chg_debug_info->highest_temp, batt_temp);
	chg_debug_info->max_iout = max(chg_debug_info->max_iout, wls_status->iout_ma);
	if (wls_status->cool_down) {
		if (!chg_debug_info->min_cool_down)
			chg_debug_info->min_cool_down = wls_status->cool_down;
		else
			chg_debug_info->min_cool_down = min(chg_debug_info->min_cool_down, wls_status->cool_down);
	}
	...
	if (cep_curr_ma) {
		if (!chg_debug_info->min_skewing_current)
			chg_debug_info->min_skewing_current = cep_curr_ma;
		else
			chg_debug_info->min_skewing_current = min(chg_debug_info->min_skewing_current, cep_curr_ma);
	}

	chg_debug_info->iout = wls_status->iout_ma;
	chg_debug_info->vout = wls_status->vout_mv;
	chg_debug_info->dock_version = wls_status->adapter_id;
	chg_debug_info->wls_auth_fail = !wls_status->verity_pass;
	chg_debug_info->work_silent_mode = wls_status->switch_quiet_mode;
	chg_debug_info->fastchg_ing = wls_status->fastchg_started;
	oplus_chg_wls_rx_get_trx_version(wls_dev->wls_rx, &trx_version);
	chg_debug_info->tx_version = trx_version;
	oplus_chg_wls_rx_get_rx_version(wls_dev->wls_rx, &rx_version);
	chg_debug_info->rx_version = rx_version;
	chg_debug_info->boot_version = 0;
	chg_debug_info->rx_temperature = 0;
	chg_debug_info->wpc_chg_err = wls_status->trx_err;
	chg_debug_info->adapter_type = wls_status->adapter_type;
	chg_debug_info->wpc_dischg_status = 0;
	chg_debug_info->break_count = wls_status->break_count;
}
...
static int oplus_chg_wls_set_trx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (en) {
		...
		oplus_wpc_update_chg_debug_info(wls_dev, false);
		oplus_chg_wireless_error(OPLUS_NOTIFY_WIRELESS_START_TX, &g_chg_debug_info);
		...
		oplus_wpc_update_chg_debug_info(wls_dev, false);
		oplus_chg_wireless_error(OPLUS_NOTIFY_WIRELESS_STOP_TX, &g_chg_debug_info);
	}
...
static void oplus_chg_wls_start_work(struct work_struct *work)
{
	...
	if (wls_status->rx_online) {
		...
		if (wpc_online_cnt >= OPLUS_CHG_WLS_START_DETECT_CNT) {
			wpc_online_cnt = 0;
			oplus_wpc_update_chg_debug_info(wls_dev, false);
			oplus_chg_wireless_error(OPLUS_NOTIFY_WIRELESS_START_CHG, &g_chg_debug_info);
...
static void oplus_chg_wls_break_work(struct work_struct *work)
{
	...
	if (wls_status->rx_online) {
		...
		oplus_chg_wireless_error(OPLUS_NOTIFY_WIRELESS_WIRELESS_CHG_END, &g_chg_debug_info);
		pr_err("wireless disconnect more than 6s, charging stop\n");
		wls_status->break_count = 0;
		oplus_wpc_update_chg_debug_info(wls_dev, true);
	}
}
...
static void oplus_chg_wls_trx_sm(struct work_struct *work)
{
	...
	if (trx_err & WLS_TRX_ERR_DEBUG_INFO_MASK) {
		oplus_wpc_update_chg_debug_info(wls_dev, false);
		oplus_chg_wireless_error(OPLUS_NOTIFY_WIRELESS_WIRELESS_CHG_BREAK, &g_chg_debug_info);
	}
