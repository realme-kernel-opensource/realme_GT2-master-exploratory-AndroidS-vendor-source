	...
	if (oplus_vooc_get_allow_reading() == false) {
		...
		if (((oplus_vooc_get_fastchg_to_normal()== true)
			...
			if((oplus_pps_get_ffc_started() == true) && (chip->fastchg_to_ffc == false)) {
				if(chip->batt_volt >= BAT_FULL_PPS_FFC_THD)
...
#define SOFT_REST_RETRY_MAX_CNT			2
#define BAT_TEMP_ABNORMAL_COOL			160
#define BAT_TEMP_ABNOMAL_WARM			450
...
static void oplus_fg_soft_reset_work(struct work_struct *work)
{
	...
	chg_err("oplus_fg_soft_reset_work\n");
	...
	if (chip->soc < SOFT_REST_SOC_THRESHOLD) {
		...
		if(oplus_gauge_check_sfr_cc() == true) {
			if(oplus_gauge_soft_reset() == 0) {
				chip->fg_soft_reset_done = true;
				chip->fg_soft_reset_fail_cnt = 0;
			} else {
...
void oplus_comm_fginfo_reset(struct oplus_chg_chip *chip)
{
	chip->fg_soft_reset_done = false;
	chip->fg_check_ibat_cnt = 0;
	chip->fg_soft_reset_fail_cnt = 0;
...
static void oplus_comm_check_fgreset(struct oplus_chg_chip *chip)
{
	...
	bool is_wls_online = true;
	bool is_temp_normal = true;

	if ((chip->tbatt_temp < BAT_TEMP_ABNORMAL_COOL)
			&& (chip->tbatt_temp > BAT_TEMP_ABNOMAL_WARM))
		is_temp_normal = false;

	if (chip->charger_type == OPLUS_CHG_USB_TYPE_UNKNOWN ||
	    chip->charger_type == OPLUS_CHG_USB_TYPE_SDP ||
	    !is_temp_normal ||
	...
	is_wls_online = oplus_chg_is_wls_present();
	if (!chip->charger_exist && !is_wls_online)
		is_need_check = false;
	...
	if (oplus_gauge_get_afi_update_done() == false) {
		chg_err("zy gauge afi_update_done ing...\n");
		is_need_check = false;
	}

	chg_err("fail_cnt[%d] reset_done [%d] temp_region[%d] full[%d %d] online[%d %d] vbat_min_mv[%d]\n",
	chip->fg_soft_reset_fail_cnt, chip->fg_soft_reset_done,
	is_temp_normal, chip->sw_full, chip->hw_full_by_sw,
	chip->charger_exist, is_wls_online, chip->batt_volt_min);
