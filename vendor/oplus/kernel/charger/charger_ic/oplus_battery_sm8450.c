...
#define NORMAL_CHG_TO_SVOOC_HANDSHAKE_DELAY_MS 100
...
static void oplus_adsp_voocphy_enable_check_func(struct work_struct *work)
{
	...
	if (voocphy_enable == 0) {
		...
		chg_err("!!Re check PMIC charging not turned on before voocphy enabled,"
		         	"chg_en:%d chg_suspend:%d", chg_enable, chg_suspend);
		if (!chg_enable || chg_suspend) {
			/**************************************************************
			 If adspvooc_phy is not eventually enabled before and need to be re-enable again here,
			 make sure PMIC normal charging turned on to draw more than 200mA ibus in advanced,
			 otherwise SVOOC adapter will not responfd to phone's handshake
			 ****************************************************************/
			oplus_chg_turn_on_charging(chip);
			mdelay(NORMAL_CHG_TO_SVOOC_HANDSHAKE_DELAY_MS);
		}
...
{
	printk(KERN_ERR "[OPPO_CHG][%s]<~WPC~> wait_wired_charge_on\n", __func__);
#if 0
...
{
#if 0
	printk(KERN_ERR "[OPPO_CHG][%s]<~WPC~> wait_wired_charge_off\n", __func__);
...
static void oplus_vchg_trig_irq_init(struct battery_chg_dev *bcdev)
{
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: vchg_trig_irq[%d]!\n", __func__, bcdev->vchg_trig_irq);
}
...
irqreturn_t oplus_vchg_trig_change_handler(int irq, void *data)
{
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: scheduling vchg_trig work!\n", __func__);
...
static int oplus_usbtemp_iio_init(struct oplus_chg_chip *chip)
{
	...
	if (rc >= 0) {
		...
		pr_err("[OPPO_CHG] test bcdev->iio.usb_temp_adc \n");
	}
	pr_err("[OPPO_CHG] test bcdev->iio.usb_temp_adc out here\n");
	...
	if (rc >= 0) {
		...
		pr_err("[OPPO_CHG] test bcdev->iio.usb_supplementary_temp_adc\n");
	}
	pr_err("[OPPO_CHG] test bcdev->iio.usb_supplementary_temp_adc out here\n");
...
static int oplus_subboard_temp_iio_init(struct oplus_chg_chip *chip)
{
	...
	if (rc >= 0) {
		...
		pr_err("[OPPO_CHG] test bcdev->iio.subboard_temp_v_chan \n");
	}
	pr_err("[OPPO_CHG] test bcdev->iio.subboard_temp_v_chan out here\n");
...
#define USBTEMP_BATTTEMP_GAP_HIGH 24
#define USBTEMP_BATTTEMP_CURRENT_GAP_HIGH 20
#define USBTEMP_MAX_TEMP_THR_HIGH 69
#define USBTEMP_MAX_TEMP_DIFF_HIGH 12
#define USBTEMP_BATTTEMP_RECOVER_GAP_HIGH 15
#define USBTEMP_BATTTEMP_CURRENT_THR_HIGH 65
#define USBTEMP_BATTTEMP_RECOVER_THR_HIGH 60
...
#define USBTEMP_BATTTEMP_CURRENT_THR_DEFAULT 54
#define USBTEMP_BATTTEMP_RECOVER_THR_DEFAULT 48
...
static void oplus_usbtemp_recover_tbatt_temp(struct oplus_chg_chip *chip) {
	...
	if ((chg_type == ADAPTER_ID_20W_0X13 || chg_type == ADAPTER_ID_20W_0X34 ||
		...
		chip->usbtemp_batttemp_current_thr = USBTEMP_BATTTEMP_CURRENT_THR_DEFAULT;
		chip->usbtemp_batttemp_recover_thr = USBTEMP_BATTTEMP_RECOVER_THR_DEFAULT;
		...
		chip->usbtemp_batttemp_current_thr = USBTEMP_BATTTEMP_CURRENT_THR_HIGH;
		chip->usbtemp_batttemp_recover_thr = USBTEMP_BATTTEMP_RECOVER_THR_HIGH;
...
static void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	if (((chip->usb_temp_r  - g_tbatt_temp/10) >= chip->usbtemp_batttemp_current_gap) || (chip->usb_temp_r >= chip->usbtemp_batttemp_current_thr)) {
		limit_cur_cnt_r++;
		if (limit_cur_cnt_r >= RETRY_COUNT) {
			limit_cur_cnt_r = RETRY_COUNT;
		}
		recover_cur_cnt_r = 0;
	} else if (((chip->usb_temp_r  - g_tbatt_temp/10) <= chip->usbtemp_batttemp_recover_gap) && (chip->usb_temp_r <= chip->usbtemp_batttemp_recover_thr))  {
		recover_cur_cnt_r++;
	...
	if (((chip->usb_temp_l  - g_tbatt_temp/10) >= chip->usbtemp_batttemp_current_gap) || (chip->usb_temp_l >= chip->usbtemp_batttemp_current_thr)) {
		limit_cur_cnt_l++;
		if (limit_cur_cnt_l >= RETRY_COUNT) {
			limit_cur_cnt_l = RETRY_COUNT;
		}
		recover_cur_cnt_l = 0;
	} else if (((chip->usb_temp_l  - g_tbatt_temp/10) <= chip->usbtemp_batttemp_recover_gap) && (chip->usb_temp_l <= chip->usbtemp_batttemp_recover_thr)) {
		recover_cur_cnt_l++;
...
/*#endif*/
	...
	if (usb_pre_plugin_status != usb_plugin_status || !usb_plugin_status) {
		if (usb_plugin_status) {
			...
			if ((oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) && (chip->vooc_project == DUAL_BATT_100W)) {
				oplus_adsp_voocphy_clear_fast_chg_type();
			}
		} else {
...
int oplus_8450_pps_cp_init(void)
{
	int status = 0;
	status = sc8571_master_subsys_init();
	status = sc8571_slave_subsys_init();
	chg_err("oplus_8450_pps_cp_init Is Initialized.\n");
...
static int fg_zy0603_check_sfr_cc(void)
{
	int rc = 0;
	struct battery_chg_dev *bcdev = NULL;
	struct psy_state *pst = NULL;
...
static int fg_bq27541_get_afi_update_done(void)
{
	int rc = 0;
	struct battery_chg_dev *bcdev = NULL;
	struct psy_state *pst = NULL;
	...
	chg_err("read afi update success, auth=%d\n", pst->prop[BATT_AFI_UPDATE_DONE]);
...
static struct oplus_gauge_operations battery_gauge_ops = {
	...
	.zy_check_sfr_cc = fg_zy0603_check_sfr_cc,
	.zy_soft_reset = fg_zy0603_soft_reset,
	.get_afi_update_done = fg_bq27541_get_afi_update_done,
};
#endif /* OPLUS_FEATURE_CHG_BASIC */
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	if (oplus_chip->vooc_project == DUAL_BATT_150W)
		oplus_pps_ops_register("cp-sc8571", &oplus_sm8350_pps_ops);
	...
	if (oplus_chip->voocphy_support == ADSP_VOOCPHY) {
		oplus_adsp_voocphy_enable(true);
	}
