...
/*		oplus_chg_sc8571_error(report_flag, NULL, 0);*/
	...
	chg_err("pps data[18~1C][%d %d %d %d %d], iic[%d, %d]\n",
	       chip->reg_dump[0], chip->reg_dump[1], chip->reg_dump[2], chip->reg_dump[3], chip->reg_dump[4],
...
static int oplus_pps_get_charging_data(struct oplus_pps_chip *chip)
{
	...
	chg_err("ap_batt_volt = %d, ap_batt_current = %d, ap_input_volt = %d, ap_input_current = %d, charger_output_volt = %d, \
		charger_output_current = %d, cp_master_ibus = %d, cp_slave_ibus = %d, vbat0 = %d\n",
			...
			chip->cp_master_ibus, chip->cp_slave_ibus, chip->vbat0);
	return 0;
}
