...
int oplus_gauge_check_sfr_cc(void)
{
	if (!g_gauge_chip)
		return -1;
	else
		return g_gauge_chip->gauge_ops->zy_check_sfr_cc();
}

int oplus_gauge_soft_reset(void)
{
	if (!g_gauge_chip)
		return -1;
	else
		return g_gauge_chip->gauge_ops->zy_soft_reset();
}

int oplus_gauge_get_afi_update_done(void)
{
	if (!g_gauge_chip)
		return -1;
	else
		return g_gauge_chip->gauge_ops->get_afi_update_done();
}
