	...
	SIPA_REG_PROC *reg, unsigned int val_width)
{
	...
	if (NULL == si_pa || NULL == reg || si_pa->regmap == NULL)
		return -EINVAL;
	...
	sipa_dev_t *si_pa)
{
	...
	if (sia91xx_selector == 1) {
		if (si_pa->channel_num == 1)
			return true;
	} else if (sia91xx_selector == 2) {
	...
	sipa_dev_t *si_pa)
{
	...
	pr_info("sipa disable sia91xx_selector:%d \r\n", sia91xx_selector);
